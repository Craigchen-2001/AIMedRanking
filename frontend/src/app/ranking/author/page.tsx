...
export default function AuthorRankingPage() {
  ...
  const [byConference, setByConference] = useState({});
  ...
  return (
    <div className="w-full min-h-screen overflow-x-hidden">
      <header className="fixed top-0 left-0 right-0 z-50 bg-red-800 h-16 flex items-center justify-between px-6 border-b">
        ...
        <div className="flex gap-3">
          <Link href="/" className="flex items-center gap-2 bg-white text-red-800 px-4 py-1.5 rounded-lg">
            <ArrowLeft size={16} /> Home
          </Link>

          <Link href="/ranking/topic" className="flex items-center gap-2 bg-white text-red-800 px-4 py-1.5 rounded-lg">
            <BookOpen size={16} /> Topic Ranking
          </Link>
        </div>
      </header>
      ...
      <div className="w-full min-h-screen overflow-x-hidden px-6 py-4 pt-5">
        {loading ? (
          ...
          <div className="grid grid-cols-12 gap-4">
            <div className="col-span-4 max-w-[580px] w-full h-full overflow-y-auto max-h-[95vh] border rounded-lg bg-white shadow-sm">
              <AuthorList ranking={ranking} />
            </div>

            <div className="col-span-8 w-full flex flex-col gap-6">
              <div className="w-full max-h-[500px] overflow-y-auto pr-2 border rounded-lg p-4 bg-white shadow-sm">
                <AuthorConferenceGrid byConference={byConference} />
              </div>

              <div className="w-full max-h-[600px] flex justify-center border rounded-lg p-4 bg-white shadow-sm">
