// // /Users/chenweichi/ICLR_2025_Project/ICLR_2025_Project/frontend/src/components/author/AuthorConferenceGrid.tsx
// "use client";
...
const CONFS_TO_SHOW: Array<(typeof ALLOWED)[number]> = [
  "ICLR",
  "ICML",
  "NEURIPS",
  "KDD",
...
function ChartByConference({
  conference,
  papers,
}: {
  conference: (typeof ALLOWED)[number];
  papers: PaperItem[];
}) {
  const years = useMemo(() => {
    const s = new Set<number>();
    papers.forEach((p) => s.add(p.year));
    return Array.from(s).sort();
  }, [papers]);
...
export default function AuthorConferenceGrid({
  ...
  return (
    <div className="">
      <h3 className="text-lg font-bold text-red-700 text-center mb-4">
        Top 10 Authors by Conference
      </h3>
      <div className="flex flex-col gap-6">
        {CONFS_TO_SHOW.map((conf) => (
          <ChartByConference key={conf} conference={conf} papers={papers} />
        ))}
      </div>
    </div>
  );
